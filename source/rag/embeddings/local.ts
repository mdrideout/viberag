...
export class LocalEmbeddingProvider implements EmbeddingProvider {
	 ...
	 * @param dtype - Model precision: 'fp16' (best quality) or 'q8' (faster, int8 quantized)
	 */
	constructor(
	 ...
	 * - fp16: Best quality (~321MB, ~99% of fp32 accuracy)
	 * - q8: Faster inference (~161MB, int8 quantized)
	 */
	...
	private async _doInitialize(): Promise<void> {
		...
		const sizeHint = this.dtype === 'fp16' ? '~321MB' : '~161MB';
		console.error(
			`[LocalEmbeddingProvider] Loading ${this.modelName} (${this.dtype}, ${sizeHint})...`,
		);
	 ...
	 * Uses batched inference for optimal memory usage.
	 * Jina v2 supports 8K tokens, so no truncation is applied.
	 */
